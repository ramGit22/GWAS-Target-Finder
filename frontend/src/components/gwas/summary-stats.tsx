...
export function SummaryStats({ results }: SummaryStatsProps) {
  ...
  const uniqueGenesCount = new Set(results.map(r => r.nearby_gene_ensembl_id)).size;
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-2xl font-bold text-blue-600">{drugTargetCount}</p>
            </div>
            <div className="space-y-1">
              <p className="text-sm text-gray-500">Within-Gene Variants</p>
              <p className="text-2xl font-bold text-green-600">{withinGeneCount}</p>
            </div>
          </div>
        </CardContent>
      </Card>
            ...
            {variantCounts.slice(0, 4).map(variant => (
              <div key={variant.type} className="space-y-1">
                <div className="flex justify-between items-center">
                  <p className="text-sm font-medium capitalize">{variant.label}</p>
                  <p className="text-sm text-gray-500">{variant.count}</p>
                </div>
                <div className="w-full bg-gray-100 rounded-full h-2">
                  <div 
                    className="bg-blue-500 h-2 rounded-full" 
                    style={{ width: `${(variant.count / results.length) * 100}%` }} 
                  />
                </div>
              </div>
            ))}
